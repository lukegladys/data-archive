...
public class Function
{
    ...
    static Function()
    {
        var startup = new ConfigureCountStartup();
        var provider = ConfigureCountStartup.ConfigureServices();

        TestService = provider.GetRequiredService<ITestService>();
    }
    ...
    /// <returns></returns>
    public static async Task<TestCustomObject> FunctionHandler(TestCustomObject customObject, ILambdaContext context)
    {
        var testString = await TestService.GetValue();
        // API Handling logic here
        return new TestCustomObject()
        {
            TestPropertyLol = testString
        };
    }
}
...
{
    public string TestPropertyLol { get; set; }
}
...
/// </summary>
[JsonSerializable(typeof(TestCustomObject))]
